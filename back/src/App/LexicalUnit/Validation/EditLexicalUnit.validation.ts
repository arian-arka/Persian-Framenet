...
import {EditLexicalUnitInterface, LexicalUnitInterface} from "../LexicalUnit.interface";
...
export default class EditLexicalUnitValidation extends Validation<EditLexicalUnitInterface,{
    ...
    custom(): Function {
        return async (data: EditLexicalUnitInterface) => {
            ...
            if(await LexicalUnitService.exists({
                type:data.type,
                name:data.name,
                frame:this.additionalData.frame._id,
                _id : {'$ne' : this.additionalData.lexicalUnit._id}
            }))
